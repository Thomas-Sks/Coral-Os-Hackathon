...
// Live status of the host-side Strix scan (written by scripts/run-strix-live.mjs) so the dashboard can
// show "the security agent is running a live pentest" before/while the market session exists.
const REPO_ROOT = join(MARKET_DIR, '..', '..')
// The session this feed is pinned to (its SESSION env) — the web auto-binds to it so opening the
// dashboard shows the live market with no ?session to paste.
app.get('/api/session', (_req, res) => res.json({ session: DEFAULT_SESSION }))

app.get('/api/scan-status', (_req, res) => {
  try {
    const s = JSON.parse(readFileSync(join(REPO_ROOT, '.scan-status.json'), 'utf8'))
    const stale = s.running && typeof s.updatedAt === 'number' && Date.now() - s.updatedAt > 25_000
    res.json(stale ? { running: false, stale: true } : s)
  } catch {
