...
/**
 * DeepSeek — OpenAI-compatible chat completions. Accepts `DEEPSEEK_API_KEY` or the generic
 * `LLM_API_KEY` (so the same key drives the Strix engine and the agents' reasoning). Default model
 * `deepseek-chat`; set `LLM_MODEL=deepseek-reasoner` for the reasoning model.
 */
async function completeDeepSeek(opts: CompleteOpts, model: string, maxTokens: number): Promise<string> {
  ...
  if (!key) throw new Error('DEEPSEEK_API_KEY (or LLM_API_KEY) not set')
  return completeOpenAICompatible(opts, model, maxTokens, {
